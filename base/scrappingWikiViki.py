...
TARGET_NAME = 'targets_vikidia.txt' #'targets_best.txt' # 'targets.txt'
...
if __name__ == '__main__':
    ...
    restart_at = None # 'Napol%C3%A9on_Ier' 'Tripoli_(Lybie)' 'TF1_games' 'BZRK'
    for vikidia in urls:
        target = vikidia.replace('/wiki/', '').strip()
        if restart_at is not None:
            if target == restart_at: restart_at = None
            else: continue
        process_target(target, "https://fr.vikidia.org/wiki/"+ target,
                       'vikidia', output=target + '.txt', strip=False)
        #process_target_both(target, output=True)
