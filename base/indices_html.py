...
def reperage_images_liens_viki(url=None, corpus=None, data=None, debug=False):
    if data is None:
    ...
    return {
        "META_NB_LINKS" : nbLiens,
        "META_NB_PICTURES" : nbImage,
        "STRUCTURE" : structure
    }

def reperage_ponctuation(url=None, corpus=None, data=None, debug=False):
    if data is None:
        req = Request("https://fr."+corpus+".org/wiki/" + url, data = None, headers={"User-Agent":'Mozilla/5.0'})
        webpage = urlopen(req)
        html_doc = webpage.read()
